#!/usr/bin/env bash
# build-android-local.sh —— 本机（/root 工具链）构建 Android APK
# 适配：JDK /root 系统 openjdk-21、Android SDK /root/android-sdk、NDK r27
set -euo pipefail

export JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64
export PATH=$JAVA_HOME/bin:$HOME/.cargo/bin:/root/flutter/bin:$PATH
...
if [ -z "$NDK" ]; then echo "!! 未找到 NDK，请先运行 setup-android-sdk.sh"; exit 1; fi
...
# 1) 第三方插件补丁（pub-cache）
PATCHED=0
patch_file() { if grep -qF "$2" "$1" 2>/dev/null; then sed -i "s|$2|$3|" "$1" && PATCHED=1; fi; }
...
[ $PATCHED -eq 1 ] && echo "[patch] 已应用插件构建补丁" || echo "[patch] 无需补丁或已就位"
